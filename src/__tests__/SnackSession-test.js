...
describe('saveAsync', () => {
  it('sends the correct data to the server', async () => {
    ...
    expect(lastCall[1]).toEqual({
      method: 'POST',
      body: `{"manifest":{"sdkVersion":"${defaultSDKVersion}"},"code":${JSON.stringify(INITIAL_CODE)}}`,
      headers: {
  ...
  it('uses name and description', async () => {
    ...
    expect(lastCall[1]).toEqual({
      method: 'POST',
      body: `{"manifest":{"sdkVersion":"${defaultSDKVersion}","name":"testname1","description":"testdescription1"},"code":${JSON.stringify(
        INITIAL_CODE
      )}}`,
      headers: {
        'Content-Type': 'application/json',

...
import './App.css';
import SnackSession from '../src1/SnackSession';
...
class App extends Component {
  constructor(props) {
    ...
    console.log(EXPO_SESSION_SECRET);
  ...
  _save = async () => {
    ...
    const appJson = this._snack.generateAppJson();
    print (appJson.expo.slug);
    const artifactUrl = await this._snack.getApkUrlAsync(appJson);
    this.setState({
  ...
  render() {
    return (
      <div>
        ...
        {this.state.artifactUrl && (
          <pre>{this.state.artifactUrl}</pre>
        )}
      </div>
    );
  }
